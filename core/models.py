...
class UserManager(BaseUserManager):
    ...
    def create_user_github(self, email, **extra_fields):
        return self.create(
            provider="GITHUB", email=email, is_active=True, **extra_fields
        )


class User(AbstractUser):
    provider = models.CharField(max_length=15, blank=True)
...
class GitHubUserViewSet(models.Model):
    def post(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {"message": "Ocorreu um erro ao salvar as informações do usuário."},
