...
from core.permissions import IsBibliotecario
from .filters import *
...
class CategoriaViewSet(ModelViewSet):
    ...
    permission_classes = [IsBibliotecario | IsAuthenticatedOrReadOnly]
...
class EditoraViewSet(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        try:
            response_editora_list_ifrn = requests.get(
                "http://127.0.0.1:8001/api/editora/"
            )
            response_editora_list_uern = requests.get(
                "http://127.0.0.1:8002/api/editora/"
            )
            response_editora_list_ufersa = requests.get(
                "http://127.0.0.1:8003/api/editora/"
            )
...
class DevolucaoViewSet(
    ...
    def devolver(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                "Empréstimo não encontrado.", status=status.HTTP_404_NOT_FOUND
            )
...
def search_books(request):
    ...
    if response.status_code != 200:
        return Response(
            {"error": "Failed to fetch data from Gutenberg API"}, status=500
        )
        ...
        resultados.append(
            {
                ...
                "autor": ", ".join(
                    [
                        author["name"]
                        for author in item.get("authors", [{"name": "Desconhecido"}])
                    ]
                ),
...
def buscar_livro(request):
    ...
    for url in urls_apis_externas:
        ...
        for item in dados_api:
            if (
                query.lower() in item.get(tipo, "").lower()
            ):  # Verifica se o resultado contém a query
                resultados.append(
                    {
